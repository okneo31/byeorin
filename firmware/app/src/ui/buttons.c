...
LOG_MODULE_REGISTER(byeorin_btn, CONFIG_LOG_DEFAULT_LEVEL);

#define OK_NODE     DT_ALIAS(ok_button)
#define CANCEL_NODE DT_ALIAS(cancel_button)

static const struct gpio_dt_spec k_btn[BYEORIN_BUTTON__COUNT] = {
#if DT_NODE_EXISTS(OK_NODE)
	[BYEORIN_BUTTON_OK]     = GPIO_DT_SPEC_GET(OK_NODE, gpios),
#endif
#if DT_NODE_EXISTS(CANCEL_NODE)
	[BYEORIN_BUTTON_CANCEL] = GPIO_DT_SPEC_GET(CANCEL_NODE, gpios),
#endif
};

K_SEM_DEFINE(m_sem, 0, 1);
static byeorin_button_cb_t m_cb;
static byeorin_button_t    m_last;

static struct gpio_callback m_cb_data[BYEORIN_BUTTON__COUNT];
static struct k_work_delayable m_debounce[BYEORIN_BUTTON__COUNT];
 ...
 *     BYEORIN_BUTTON_EV_RELEASE iff level==0 after the same window).
...
static void debounce_handler(struct k_work *w)
{
	...
	for (int i = 0; i < BYEORIN_BUTTON__COUNT; i++) {
		if (dw != &m_debounce[i]) { continue; }
		if (!device_is_ready(k_btn[i].port)) { return; }
		int level = gpio_pin_get_dt(&k_btn[i]);
		if (level == 1) {
			m_last = (byeorin_button_t)i;
			if (m_cb) { m_cb(m_last, BYEORIN_BUTTON_EV_PRESS); }
			k_sem_give(&m_sem);
		}
	}
}
...
void buttons_register_cb(byeorin_button_cb_t cb) { m_cb = cb; }
