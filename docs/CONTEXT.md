...
- **BTC 페깅 해제** (2026-07-29). 옛 결정 "TTL = 10/365 BTC"(2026-07-25, `a4b4a03`)는
  **2026-07-29 앵커로 한 번 쓰고 해제됐다.** BTC 63,412.45 @ 2026-07-29 00:00 KST 를
  ...
  > 대신 화면은 TTL 잔액을 **환산하지 않는다** — TTL 잔액 옆에는 그 정의
  > (`노동자 {v} 일 품삯`, `tokens.value_labor_days`)가 곱셈 0 회로 그대로 온다.
  > 두 앵커가 7 배 어긋나 보이던 문제(`63,412.45 × 10/365 ≈ 1,737 USD/TTL` 대
  > `1 TTL = 246.65 tUSD`, 커밋 `c9a6771`)는 **TTL 쪽으로 들어오는 시세 경로가
  > 사라져** 해소됐다.
- **모든 자산을 TTL 로 잰다** (2026-08-04). 상장자산(BTC·ETH·SOL·TRX·TON·APT·SUI·
  XRP…)의 주 표시가 USD 에서 **TTL** 로 바뀐다. 산식은 한 방향뿐이다 —
  `시장 USD 시세 × (1 USD 당 TTL) = 그 자산의 TTL 값`. 계수 `1 USD 당 TTL` 은
  tUSD `perTtl` 의 역수이고, **런타임에 `rate-snapshot.json`(→ `rateByIso('USD')`)
  에서 읽는다.** 이 문서에도 코드에도 그 숫자를 적지 않는다 — 아래 금지선 (b).
  > **유지되는 것이 먼저다 — TTL 의 값은 어디서도 시장에서 오지 않는다.** TTL 의
  > 눈금은 `perTtl`(명목GDP ÷ 인구 ÷ 365) 하나로만 정해지고, 이번 변경은 그
  > 산식에 입력을 하나도 더하지 않았다. TTL 잔액 옆에 오는 것은 여전히 자기 정의
  > (`tokens.value_labor_days`) 뿐이고 곱셈은 0 회다.
  > **이것은 페그의 재발이 아니다.** 앞 항목(BTC 페깅 해제)이 금지한 것은 *TTL
  > 자신의 값이 시장을 따라가는 것*(`TTL 잔액 × BTC 시세`)이다. 이번에 생긴
  > 화살표는 반대 방향이다 — 다른 자산을 TTL 이라는 **자로 재는 것**이고, 재어
  > 진다고 해서 자의 눈금이 바뀌지 않는다. 시장 시세는 재어지는 자산의 성질이지
  > 자의 성질이 아니다.
  > **금지선 (a) — TTL 잔액에 어떤 시세도 곱하지 않는다.** 허용된 곱셈은
  > `상장자산 USD 시세 × USD당 TTL` 하나뿐이다. 좌변에 TTL 잔액이 오는 곱셈이
  > 코드에 나타나면 그것이 위반이다. 방향이 뒤집히면 이 문단을 근거로 되돌린다.
  > 지금은 타입이 이것을 막는다 — `AssetRef` 의 `'ttl'` 갈래에는 `symbol` 이
  > 없어서 시세표에 넘길 인자가 없고, `assetValueInTtl` 은 그 분기에서 즉시
  > 반환한다(`packages/wallet-sdk/src/rates/value.ts`).
  > **금지선 (b) — 환율을 코드에 상수로 박지 않는다.** `perTtl` 도, 그 역수도,
  > 어떤 통화의 환산 계수도 마찬가지다. 이유는 값이 고정이 아니기 때문이다 —
  > `scripts/build-rate-snapshot.mjs` 를 다시 돌리면 스냅샷 전 종목이 바뀐다.
  > 스냅샷에 `anchoredAt` 이 박혀 있는 이유가 그것이다. 상수는 재앵커 순간
  > 거짓이 되고, 스냅샷과 두 벌이 되어 사실상 새 페그가 된다.
  > ① 값은 **런타임에 스냅샷에서 읽는다** (하드코딩·기본값·폴백 상수 전부 금지).
  > ② 스냅샷을 못 읽거나 그 통화가 없으면 **비운다.** 옛 값으로 떨어지면 조용히
  > 틀린 금액이 뜬다 — 빈칸은 보이지만 틀린 숫자는 안 보인다.
  > ③ 화면에 값을 보일 때 **어느 앵커의 값인지**(`anchoredAt`)를 알 수 있게 한다.
  > ④ **테스트로 고정한다** — 소스에 환율 숫자 리터럴이 없다는 것, 그리고
  > 스냅샷 값을 바꾸면 결과가 따라 바뀐다는 것 두 가지를.
  > **고정 액면과 시세 기준은 화면에서 구분된다.** t{ISO}·스테이블의 TTL 값은
  > 스냅샷 고정이라 움직이지 않고, 상장자산의 TTL 값은 시세를 타 출렁인다. 구분이
  > 없으면 출렁임이 "TTL 이 BTC 를 따라간다"로 오독돼 방금 지운 페그와 화면상
  > 구별이 안 된다.
  > **값을 모르면 비운다.** 시세를 못 얻으면 `tokens.value_unlisted`, 주소로 신원이
  > 확인되지 않으면 `tokens.value_unverified`. 뭉뚱그리지 않는다. 포트폴리오 TTL
  > 합계에 값 미상 자산이 섞이면 그 사실을 함께 표시한다 — 빠진 것을 숨긴 합계는
  > 거짓이다.
  > 산식은 `packages/wallet-sdk/src/rates/` 순수 함수 한 곳에만 둔다. 셸 4 종은
  > 부르기만 한다(v0.5.21 에 셸마다 짜서 4 벌이 어긋났다).
  > 전문은 [`docs/RATES.md §2.1`](./RATES.md).
- **벼린 환율 = TTL 이 기준, 시장환율을 입력으로 쓰지 않는다** (2026-07-29).
  ...
  GDP·인구 → 통화토큰 환율)는 별개 트랙이고 만나지 않는다. "만나지 않는다"는
  **앵커**(TTL 의 값이 어디서 오는가)에 대한 말이지 **측정**(다른 자산을 TTL 로
  표현하는 것)에 대한 말이 아니다 — 측정은 2026-08-04 에 허용됐다(위 항목).
