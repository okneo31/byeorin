...
## [v0.5.23] 모든 자산을 TTL 로 잰다 — 2026-08-04

안드로이드 `versionCode 24 / versionName 0.5.23`.

### Changed — BTC·ETH·SOL 도 달러가 아니라 TTL 로 보인다
- 지금까지 상장 자산은 `≈ $63,412` 처럼 **달러**로 보였다. 이제 `≈ 257.1 TTL` 처럼 **TTL** 로 보인다. 달러 보조줄 자리를 TTL 이 대체한다.
- 이유는 하나다. **TTL 이 기준이다.** 달러로 재던 것을 TTL 로 재는 쪽으로 방향을 되돌린 것이다. 스테이블코인(v0.5.21)·통화토큰에 이어 상장 자산까지 같은 자(尺)로 재어진다.
- **TTL 자신은 환산하지 않는다.** TTL 잔액에는 어떤 시세도 곱하지 않고 자기 정의(노동자 며칠 품삯)만 표시한다. 거래소에 TTL 페어가 생겨도 마찬가지다 — 지갑이 그 값을 집지 않도록 코드에서 막아 두었다.

### Added — 값이 어디서 왔는지 화면에서 구분된다
- **고정 액면**(스테이블코인·t{ISO} 통화토큰)과 **시세 기준**(BTC 같은 상장 자산)을 행마다 배지로 구분한다. 시세 기준 값은 시장이 움직이면 **같이 출렁인다.** 고정 액면은 출렁이지 않는다.
- 출렁이는 것은 재어지는 자산이지 TTL 눈금이 아니다. BTC 값이 오르내려도 1 TTL 이 뜻하는 하루 품삯은 그대로다.
- **포트폴리오 합계**를 TTL 로 낸다. 값을 모르는 자산은 합계에 넣지 않고 **몇 건이 빠졌는지 함께 적는다.** 빠진 것을 숨기고 합계를 내지 않는다.
- 화면에 **환율 기준 시점(anchoredAt)** 을 표시한다. 지금 보이는 TTL 값이 어느 앵커의 환율로 계산됐는지 알 수 있다.

### Note — 환율은 상수가 아니다
- TTL 환산에 쓰는 환율표는 각국 명목 GDP ÷ 인구 ÷ 365(하루 품삯)로 계산된 **스냅샷**이다. 원천 통계가 갱신되어 스냅샷을 다시 만들면 **66 종 환율이 전부 바뀌고, 화면의 TTL 값도 따라 바뀐다.**
- 그래서 환율 숫자는 코드 어디에도 박혀 있지 않다. 값은 실행할 때 스냅샷에서 읽고, 못 읽으면 **옛 값으로 떨어지지 않고 비운다.** 조용히 틀린 금액을 보여 주는 것보다 비우는 쪽이 낫다.

### Changed — 내부 정리 (계산 결과는 같다)
- 자산 → TTL 환산 산식을 셸 4 종에 흩어져 있던 조각들에서 지갑 공용 함수 하나로 모았다. 화면마다 다른 수가 나오던 여지를 없앴다.
- 값을 매기는 대상은 여전히 **주소로 신원이 확인된** 토큰뿐이다(v0.5.20 규칙 유지). 이름만 USDT 인 가짜 토큰은 값을 얻지 못한다.

---

## [v0.5.22] 확장 프로그램이 스테이블코인 자릿수를 남의 말로 정하지 않는다 — 2026-08-04

안드로이드 `versionCode 23 / versionName 0.5.22`.

### Fixed — 확장 프로그램의 금액이 10 배·1000 배로 어긋날 여지를 없앴다
- 지금까지 확장 프로그램은 USDT 같은 토큰의 **소수점 자릿수(decimals)** 를 외부 익스플로러가 준 값 그대로 믿었다. 그 값이 틀리면 화면 금액이 10ⁿ 배로 어긋난다.
- 이제 자릿수를 지갑이 한 번만 정하고, 수량 표시와 TTL 환산이 **같은 자릿수**를 쓴다. 한 줄 안에서 두 숫자가 다른 기준으로 계산되는 일이 없어졌다.
- 값이 바뀌는 상황: 익스플로러가 틀린 자릿수를 줬을 때만이다. 정상 응답에서는 화면 숫자가 이전과 같다.

### Changed — 내부 정리 (화면에 보이는 변화 없음)
- 스테이블코인 판정·TTL 환산 산식이 셸마다 따로 있던 것을 지갑 공용 코드 한 곳으로 모았다. 계산 결과는 이전과 같다 — USDT 100 개 = 0.4054 TTL 그대로다.
- 컨트랙트 주소를 화면 코드에 베껴 두던 것을 없앴다. 주소는 이제 공용 목록 한 곳에만 있다.
- 안드로이드에서 심볼 이름만 보고 `$1.00` 을 주던 잔재를 지웠다. 주소로 신원이 확인된 토큰만 값을 받는 규칙(v0.5.20)에 맞춘 것이다. 이로 인해 ZION 의 `uusdt` 는 값 대신 사유가 표시된다.
- 화면에 쓰이지 않는 문구 5 종을 번역 파일에서 뺐다.

---

## [v0.5.21] 스테이블코인이 달러가 아니라 TTL 로 재어진다 — 2026-08-04

안드로이드 `versionCode 22 / versionName 0.5.21`.

### Changed — USDT·USDC·DAI 의 값이 TTL 로 표시된다
- 지금까지 스테이블코인은 `≈ $100.00` 처럼 **달러**로 보였다. 이제 `≈ 0.4054 TTL` 처럼 **TTL** 로 보인다.
- 이유는 하나다. **TTL 이 기준이다.** 달러도 원화도 엔화도 TTL 로 재는 것이지, TTL 을 달러로 재는 것이 아니다. 달러 스테이블은 달러를 담은 토큰이므로 다른 통화와 똑같이 TTL 로 재어진다.
- 환산에 쓰는 값은 지갑 안에 들어 있는 벼린 환율표(각국 명목 GDP ÷ 인구 ÷ 365 = 하루 품삯)다. 예를 들어 1 TTL = 246.65 달러이므로 **USDT 100 개 = 0.4054 TTL** 이다. 시장 시세를 보지 않기 때문에 거래소가 닫혀 있어도, 인터넷 시세를 못 받아와도 이 값은 그대로다.
- 값을 매기는 대상은 **주소로 신원이 확인된** 스테이블코인뿐이다(v0.5.20 에서 세운 규칙 그대로). 이름만 USDT 인 가짜 토큰은 달러도 TTL 도 얻지 못하고 `값 모름 · 신원 미확인` 으로 남는다.
- 액면 통화를 모르는 스테이블은 값을 비우고 사유를 밝힌다. 추측해서 채우지 않는다.

### Unchanged — BTC·ETH·SOL 등 상장 자산은 그대로 달러 보조줄
- 상장 자산의 달러값은 거래소의 **시장 시세**다. 시장 시세를 TTL 환율에 섞으면 벼린 환율이 시장을 타게 된다. 그래서 섞지 않는다.
- 스테이블코인이 다른 이유: 1 USDT 는 발행자가 **1 달러라고 선언한 액면**이지 시세가 아니다. 액면은 시장을 타지 않는다. 이 구분이 이번 변경의 전부다.
- 웹·데스크톱 화면은 토큰 가치 줄 자체가 없어 이번 변경에 해당 사항이 없다. 표시가 바뀌는 곳은 안드로이드·확장 2 종이다.

---

## [v0.5.20] 가짜 USDT 가 1 달러로 보이지 않는다 — 2026-08-04

안드로이드 `versionCode 21 / versionName 0.5.20`.

### Fixed — 이름만 USDT 인 토큰이 개당 1 달러로 찍히던 문제
- 지금까지 지갑은 토큰의 **이름(심볼)만 보고** 값을 매겼다. USDT·USDC·DAI 라고 적혀 있으면 개당 1 달러로 계산했다.
- 그런데 토큰 이름은 누구나 지을 수 있다. 아무나 토큰을 만들어 이름을 `USDT` 로 붙이고 100 만 개를 보내면, 화면에 **100 만 달러**로 떴다. 자기 재산을 크게 잘못 알 수 있었다.
- 이제 이름이 아니라 **컨트랙트 주소**로 판단한다. 지갑이 알고 있는 진짜 주소(체인별 USDT·USDC·DAI 등, TRON 의 정본 USDT 포함)일 때만 값을 매긴다.
- 주소를 모르는 토큰은 값을 **비워 둔다.** `값 모름 · 신원 미확인` 이라고 밝히고, 왜 그런지 설명을 붙인다. 수량과 이름은 그대로 보이므로 자산이 사라지는 것이 아니다.
- 대문자·소문자·공백·비슷하게 생긴 외국 글자로 이름을 바꿔치는 수법도 함께 막힌다. 이름이 값 판단에 아예 쓰이지 않기 때문이다.

### Changed — TTL 옆에 달러 환산이 사라졌다 (웹·데스크톱)
- 웹과 데스크톱 잔액 화면에 아직 남아 있던 `≈ $… USD` 줄을 지웠다.
- **TTL 은 TTL 이다.** 값을 다른 통화로 재지 않는다. 그 자리에 TTL 자신의 정의인 **노동자 N 일 품삯**이 온다. 4 종(웹·확장·안드로이드·데스크톱) 표기가 이제 같다.
- 외부 상장 자산(BTC·ETH·SOL 등)의 시세 보조줄은 그대로다.

---

## [v0.5.19] TRON 의 USDT 가 주소 대신 이름으로 뜬다 — 2026-08-04

안드로이드 `versionCode 20 / versionName 0.5.19`.

### Fixed — TRON 토큰이 이름 대신 컨트랙트 주소로 보이던 문제
- TRON 자산 목록에서 USDT 가 `TR7NHq…gjLj6t` 같은 축약 주소로 떴다. 무키 TronGrid 는 IP 할당량이 작아, 토큰마다 이름까지 물어보면 할당량이 먼저 바닥나 금액(자릿수)조차 못 읽는다. 그래서 예산을 자릿수에 몰아주고 이름표를 포기해 온 절충이었다.
- 이제 주소가 고정된 유명 TRC-20 토큰은 **네트워크를 한 번도 쓰지 않고** 지갑 안에 적힌 목록에서 이름을 붙인다. 할당량을 전혀 쓰지 않으므로 금액 표시가 나빠지지 않는다. TRC-20 USDT 는 `USDT / Tether USD` 로 뜬다.
- 자릿수는 여전히 **체인에서 읽은 값이 우선**이다. 내장 목록과 체인 값이 다르면 체인 값을 쓴다 — 지갑이 적어 둔 값 때문에 금액이 틀리게 보이는 일은 없다.
- 목록에 없는 토큰은 지금까지와 똑같이 축약 주소로 남는다. **모르는 이름을 지어내지 않는다.**
- 이름 조회를 켠 경우(기본 꺼짐)에도 호출 방식이 바뀌었다: 자릿수 조회가 모두 끝난 뒤에, 잔액이 큰 토큰 몇 개에만, 순차로 심볼만 물어본다. 이름 때문에 금액 조회가 밀려나지 않게 하기 위해서다.
