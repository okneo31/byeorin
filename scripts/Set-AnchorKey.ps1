...
# PowerShell 5.1 의 $OutputEncoding 기본값은 ASCII 다. 그대로 두면 네이티브
# 프로세스(node)로 파이프되는 한국어 시드구문이 '?' 로 뭉개져 "유효하지 않은
# 시드" 로 죽는다. 영어 wordlist 만 쓰면 안 걸리는, 한국어에서만 터지는 함정이다.
