...
export function TokenListPane({
  ...
  // 펼쳐 놓은 행의 key(소문자 주소). 한 번에 하나만 — 팝업 폭 420 에서 여러 개가
  // 펼쳐지면 목록의 형태를 잃는다.
  const [expanded, setExpanded] = useState<string | null>(null);

  // 가리기 상태 — chrome.storage.local. shell-core 의 백엔드를 그대로 재사용한다
  // (주소록/키스토어와 같은 저장 계층).
  const backend = useMemo(() => new ChromeLocalBackend(), []);
  ...
  // 저장 실패(권한 없음 등)해도 화면은 메모리 상태로 계속 동작한다. saveHidden 이
  // false 를 돌려주면 그 사실만 알린다 — 조작 자체를 막지는 않는다.
