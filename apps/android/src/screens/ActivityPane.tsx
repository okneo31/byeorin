          ...
          {items.map((it, i) => {
            ...
            return (
              ...
              <li key={`${it.hash}-${it.blockNumber.toString()}-${i}`} className="activity-row">
                 ...
                 * 문자열이라 **HTML 을 만들지 않는다** — dangerouslySetInnerHTML
                 * 금지. renderMemo 는 문자열 조각 배열(splitMemoLinks)을 React
                 * 노드로만 바꾸고, http(s) URL 만 <a> 가 된다. href 와 화면 글자가
                 * 같은 값이라 링크 위장이 생기지 않는다. */}
                {memo !== null && (
                  <div className="activity-row__line activity-row__line--memo">
                    <span className="muted small">{t('activity.memo_label')}</span>
                    <span
                      className={`activity-row__memo small${memoHasLink(memo) ? ' activity-row__memo--has-link' : ''}`}
                    >
                      {renderMemo(memo)}
                    </span>
                  </div>
